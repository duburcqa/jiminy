...
namespace jiminy::python
{
    ...
    public:
        ...
        {
            const PyTypeObject * pytype =
                reinterpret_cast<PyArray_Descr *>(np::dtype::get_builtin<T>().ptr())->typeobj;
    ...
    struct stringFromPython
    {
        ...
        static unaryfunc * get_slot(PyObject * obj)
        {
            return (PyUnicode_Check(obj)) ? &py_unicode_as_string_unaryfunc :
                   PyBytes_Check(obj)     ? &py_object_identity :
                                            0;
        };
        ...
        static const PyTypeObject * get_pytype() { return &PyUnicode_Type; }
    };
    ...
    public:
        nativeFromPython()
        {
            bp::converter::registry::insert(&nativeFromPython<T, SlotPolicy>::convertible,
    ...
    void exposeCompatibility()
    {
        ...
        /* Add native string from python converter to resolve old VS new cxx11 string ABI conflict.
           Since it is impossible to guarantee other Boost.Python extension modules will be
           compiled with the same ABI, it is necessary to register manually the missing Native
           Python types converters, since they are normally only defined once by the first module
           to be loaded. Old and new ABI are different objects in practice, so there will be no
           conflicts. For references about this issue:
