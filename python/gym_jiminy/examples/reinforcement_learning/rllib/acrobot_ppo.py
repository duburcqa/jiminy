...
register_env("train", lambda env_config: gym.make(GYM_ENV_NAME, **env_config))
...
algo_config.python_environment(
    ...
    extra_python_environs_for_worker = {
        # Disable multi-threaded linear algebra, ensuring one thread per worker
        "OMP_NUM_THREADS": "1"
    }
)
...
algo_config.evaluation(
    ...
    evaluation_config=dict(
        ...
        # Real-time rendering during evaluation (as opposed to post-processing)
        render_env=False,
        explore=False,
        horizon=None
    )
...
algo_config.framework(
    # Use PyTorch ("torch") instead of Tensorflow ("tf" or "tfe" (eager mode))
    framework="torch"
)
...
algo_config.training(
    ...
    use_gae=True,
    # Whether to user critic as a value baseline (required for computing GAE)
    use_critic=True,
...
algo_config.training(
    ...
    # PPO clip parameter
    clip_param=0.2,
    # Clip param for the value function (sensitive to the scale of the rewards)
