...
class FrameStackingModel(TFModelV2):
    ...
    def __init__(self,
        ...
        self.sensor_space_size = flatdim(obs_space.original_space["sensors"])
        ...
        if self.num_frames > 1:
            self.view_requirements["n_obs"] = ViewRequirement(
                data_col=SampleBatch.OBS,
                shift="-{}:0".format(num_frames - 1),  # Last included !
                space=obs_space)
        ...
        stack_size = self.sensor_space_size + action_space.shape[0] + 1
        ...
        if self.num_frames >= 16:
            ...
            features = tf.keras.layers.Flatten(name="flatten")(
                stack[..., :-1, :])
            concat = tf.keras.layers.Concatenate(
                axis=-1, name="concat")([obs, features])
        else:
    ...
    def forward(self,
        ...
        if self.num_frames > 1:
            stack = tf.concat((
                input_dict["n_obs"][..., :self.sensor_space_size],
            ...
            inputs = obs
        out, self._last_value = self.base_model(inputs)
        return out, states
