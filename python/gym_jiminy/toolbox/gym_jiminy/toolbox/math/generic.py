...
def quat_to_rpy(quat: np.ndarray) -> np.ndarray:
    ...
    rpy = np.stack((roll.T, pitch.T, yaw.T), axis=-1)
    if quat.ndim == 1:
        return rpy[0]
    return rpy
...
def matrix_to_quat(matrix: np.ndarray) -> np.ndarray:
    ...
    :warning: The function assumes unit rotation matrix.
    """
    assert matrix.ndim >=2
    if matrix.ndim==2:
        mat = np.atleast_3d(matrix).T
    else:
        mat = matrix[:]
    quat = np.stack((
        mat[..., 2, 1] - mat[..., 1, 2],
...
def rpy_to_matrix(rpy: np.ndarray) -> np.ndarray:
    ...
    col_1 = np.stack(((c_p * c_y).T, (c_p * s_y).T, -s_p.T), -1)
    col_2 = np.stack((
        (-c_r * s_y + s_r * s_p * c_y).T,
        (c_r * c_y + s_r * s_p * s_y).T,
        (s_r * c_p).T
        ), -1)
    col_3 = np.stack((
        (s_r * s_y + c_r * s_p * c_y).T,
        (-s_r * c_y + c_r * s_p * s_y).T,
        (c_r * c_p).T
        ), -1)
    matrix = np.stack((col_1, col_2, col_3), -1)
    if rpy.ndim == 1:
