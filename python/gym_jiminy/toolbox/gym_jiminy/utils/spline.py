...
class _solve_banded(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        ctx.band = band
        ctx.save_for_backward(Mb, x)

        return x.to(b.device)

    @staticmethod
    def backward(ctx, grad_f_x):
        Mb, x = ctx.saved_tensors

        Mtb = torch.zeros_like(Mb)
        for i, j in enumerate(range(ctx.band[0], 0, -1)):
            Mtb[..., i, j:].copy_(Mb[..., ctx.band[0] + ctx.band[1] - i, :-j])
        Mtb[..., ctx.band[0], :].copy_(Mb[..., ctx.band[1], :])
        for i, j in enumerate(range(-1, - ctx.band[1] - 1, -1)):
            Mtb[..., ctx.band[0] + 1 + i, :j].copy_(
                Mb[..., ctx.band[1] - 1 - i, -j:])

        grad_f_b = _solve_banded.apply(
            [ctx.band[1], ctx.band[0]], Mtb, grad_f_x)
        grad_f_Mb = torch.zeros_like(Mb)
        for i, j in enumerate(range(-ctx.band[1], ctx.band[0] + 1)):
            ...
            if d.ndim == Mb.ndim:
                d = torch.sum(d, dim=-2)
            if j > 0:
                grad_f_Mb[..., i, :-j].copy_(d)
            else:
                grad_f_Mb[..., i, -j:].copy_(d)
        return None, grad_f_Mb, grad_f_b


class _solveh_banded(torch.autograd.Function):
        ...
        if Mb.ndim > 2:
            ...
            for ind in zip(*[grid_i.flatten() for grid_i in grid]):
                x[ind] = torch.from_numpy(scipy.linalg.solveh_banded(
                    Mb_np[ind], b_np[ind], lower=False,
                    overwrite_ab=False, overwrite_b=False, check_finite=False))
        else:
            x = torch.from_numpy(scipy.linalg.solveh_banded(
                Mb_np, b_np, lower=False,
                overwrite_ab=False, overwrite_b=False, check_finite=False))
        if Mb.ndim == b.ndim:
            x.transpose_(-2, -1)
    ...
    def backward(ctx, grad_f_x):
        Mb, x = ctx.saved_tensors
        grad_f_b = _solveh_banded.apply(ctx.band, Mb, grad_f_x)
        grad_f_Mb = torch.zeros_like(Mb)
...
class Spline(nn.Module):
    ...
    def __init__(self,
        ...
        if dydt is None:
            ...
            Mb = torch.zeros((*t.shape[:-1], 3, t.shape[-1]),
            ...
            # breaks[1], ..., breaks[N-1]
            Mb[..., 0, 2:].add_(idt[..., 1:])
            Mb[..., 1, 1:-1].add_(idt[..., :-1] + idt[..., 1:], alpha=2)
            Mb[..., 2, :-2].add_(idt[..., :-1])
            b[..., 1:-1] = 3 * (dy[..., :-1] * idt2[..., :-1] +
                                dy[..., 1:] * idt2[..., 1:])
            ...
            if mode == 'periodic':
                ...
                Mb[..., 0, 1].add_(idt[..., -1])
                Mb[..., 1, 0].add_(idt[..., -1] + idt[..., 0], alpha=2)
                M_0_m1 = idt[..., 0].clone()
                b[..., 0] = 3 * (dy[..., -1] * idt2[..., -1] +
                                 dy[..., 0] * idt2[..., 0])
                ...
                v[..., 0, 0] = 1.0
                v[..., -1, 0] = - M_0_m1 / Mb[..., 1, 0].clone()
                Mb[..., 1, -1] += M_m1_0 * M_0_m1 / Mb[..., 1, 0].clone()
                Mb[..., 1, 0] += Mb[..., 1, 0].clone()
                if b.ndim > 1:
                    Mb.squeeze_(-3)
                ...
                s = _solveh_banded.apply(1, Mb[..., :-1, :], b_u)
                if Mb.ndim > b.ndim:
                    d, q = s[..., 0, :], s[..., 1, :]
                else:
                    d, q = s[..., :-1, :], s[..., [-1], :]
                ...
                c1 = M_0_2.clone() / Mb[..., 0, 2].clone()
                c2 = M_m1_m3.clone() / Mb[..., 2, -3].clone()
                Mb[..., 1, 0].sub_(c1 * Mb[..., 2, 0].clone())
                Mb[..., 0, 1].sub_(c1 * Mb[..., 1, 1].clone())
                Mb[..., 2, -2].sub_(c2 * Mb[..., 1, -2].clone())
                Mb[..., 1, -1].sub_(c2 * Mb[..., 0, -1].clone())
                ...
                if b.ndim > 1:
                    Mb.squeeze_(-3)
                dydt = _solve_banded.apply((1, 1), Mb, b)
            elif mode == 'natural':
                ...
                # Condition on continuity of the 2nd derivative at breaks[0]
                Mb[..., 1, 0].add_(idt[..., 0], alpha=2)
                Mb[..., 0, 1].add_(idt[..., 0])
                b[..., 0] = 3 * dy[..., 0] * idt2[..., 0]

                # Condition on continuity of the 2nd derivative at breaks[N-1]
                Mb[..., 2, -2].copy_(idt[..., -1])
                Mb[..., 1, -1].copy_(2 * idt[..., -1])
                b[..., -1] = 3 * dy[..., -1] * idt2[..., -1]

                # Compute the derivative at each breaks
                if b.ndim > 1:
                    Mb.squeeze_(-3)
                dydt = _solveh_banded.apply(1, Mb[..., :-1, :], b)
    ...
    def forward(self,
        ...
        if orders is None:
            orders = torch.tensor([0], device=ts.device)
        ...
        with torch.no_grad():
            ...
            Ind.clamp_(min=0, max=self.t.shape[-1] - 2)
            if Ind.ndim > 1:
                Ind = Ind[..., 0, :]
        ...
        if Ind.ndim > 1:
            t_Ind = torch.stack([
                self.t[j, ..., i] for j, i in enumerate(Ind)], dim=0)
            dt_Ind = torch.stack([
                self.dt[j, ..., i] for j, i in enumerate(Ind)], dim=0)
            p_Ind = torch.stack([
                self.p[j, ..., i] for j, i in enumerate(Ind)], dim=0)
        else:
            t_Ind = self.t[..., Ind]
            dt_Ind = self.dt[..., Ind]
            p_Ind = self.p[..., Ind]
        ...
        ratio = (ts - t_Ind) / dt_Ind
        ys = torch.stack([
            torch.sum(p_Ind * self._h_poly(ratio, o), dim=-2) / dt_Ind ** o
