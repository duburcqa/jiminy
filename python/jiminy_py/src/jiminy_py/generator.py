...
def get_random_tile_ground(tile_size: np.ndarray,
    ...
    @nb.jit(
        nb.types.void(nb.float64,
                      nb.float64,
                      nb.types.Array(nb.float64, 0, 'C'),
...
def sum_tile_ground(heightmaps: Sequence[HeightMapFunctor]
                   ) -> HeightMapFunctor:
    # Extract python methods for fast access
    heightmaps_py = nb.typed.List.empty_list(
        nb.types.void(nb.float64,
           nb.float64,
           nb.types.Array(nb.float64, 0, 'C'),
           nb.types.Array(nb.float64, 1, 'C')
    ...
    for heightmap in heightmaps:
        ...
        if heightmap_py is None or not isinstance(
                heightmap_py, nb.core.dispatcher.Dispatcher):
            raise ValueError(
                "'heightmaps' must only contained heightmaps generated by "
                "`get_random_tile_ground` method.")
        heightmaps_py.append(heightmap_py)
    ...
    def _sum_tile_ground_impl(x: float,
                              ...
                              heightmaps_py: nb.typed.List) -> None:
    ...
    return HeightMapFunctor(
        partial(_sum_tile_ground_impl,
                height_buffer=np.array(0.0),
                normal_buffer=np.empty((3,)),
                heightmaps_py=heightmaps_py),
        heightMapType_t.GENERIC)
