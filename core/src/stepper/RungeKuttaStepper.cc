...
namespace jiminy
{
    ...
    {
        // TODO assert sizes
        // TODO: the number of points needed for the numerical scheme is known in advance so
        // maybe this could be preallocated better.
    }

    bool RungeKuttaStepper::try_step_impl(state_t           & state,
                                          ...
                                          float64_t         & dt)
    {
        ...
        for (uint32_t i = 1; i < c_.size(); ++i)
        {
            ...
            ki.push_back(fWrapper(t + c_[i] * dt, xi));
        }
        ...
        // Step failed if normalizedError is larger than 1.
