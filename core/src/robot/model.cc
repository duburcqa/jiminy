...
namespace jiminy
{
    ...
    hresult_t Model::initialize(const pinocchio::Model & pinocchioModel,
                                ...
                                const pinocchio::GeometryModel & visualModel)
    {
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            getJointTypeFromIndex(pinocchioModel, 1, rootJointType);  // Cannot fail
            urdfPath_ = "";
            urdfData_ = "";
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            addConstraints(jointConstraintsMap,
                           ConstraintNodeType::BOUNDS_JOINTS);  // Cannot fail at this point
        }
                                ...
                                bool loadVisualMeshes)
    {
        ...
        returnCode = buildMultipleModelsFromUrdf(urdfPath,
                                                 hasFreeflyer,
                                                 meshPackageDirs,
                                                 pinocchioModel,
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            returnCode = initialize(pinocchioModel, pinocchioCollisionModel, pinocchioVisualModel);
        }
                              ...
                              const pinocchio::FrameType & frameType)
    {
        ...
        pinocchio::FrameIndex parentFrameIndex = 0;
        if (returnCode == hresult_t::SUCCESS)
        {
            returnCode = getFrameIndex(pinocchioModelOrig_, parentBodyName, parentFrameIndex);
        }
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            {
                getFrameIndex(pncModelFlexibleOrig_,
                              parentBodyName,
                              parentFrameIndex);  // Cannot fail at this point
                pinocchio::JointIndex parentJointIndex =
    ...
    hresult_t Model::removeFrames(const std::vector<std::string> & frameNames)
    {
        ...
        for (const std::string & frameName : frameNames)
        {
            ...
            const pinocchio::FrameType frameType = pinocchio::FrameType::OP_FRAME;
            returnCode = getFrameIndex(pinocchioModelOrig_, frameName, frameIndex);
            if (returnCode == hresult_t::SUCCESS)
            {
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            for (const std::string & frameName : frameNames)
            {
                ...
                pinocchio::FrameIndex frameIndex;
                getFrameIndex(
                    pinocchioModelOrig_, frameName, frameIndex);  // Cannot fail at this point
                ...
                getFrameIndex(pncModelFlexibleOrig_,
                              frameName,
                              frameIndex);  // Cannot fail at this point
    ...
    hresult_t Model::addContactPoints(const std::vector<std::string> & frameNames)
    {
        ...
        returnCode = addConstraints(frameConstraintsMap, ConstraintNodeType::CONTACT_FRAMES);
    ...
    hresult_t Model::removeContactPoints(const std::vector<std::string> & frameNames)
    {
        ...
        if (!frameNames.empty())
        {
            removeConstraints(frameNames,
                              ConstraintNodeType::CONTACT_FRAMES);  // Cannot fail at this point
    ...
    hresult_t Model::addConstraints(const ConstraintMap & constraintMap, ConstraintNodeType node)
    {
        hresult_t returnCode = hresult_t::SUCCESS;
    ...
    hresult_t Model::removeConstraint(const std::string & constraintName, ConstraintNodeType node)
    {
    ...
    hresult_t Model::getConstraint(const std::string & constraintName,
                                   std::shared_ptr<AbstractConstraintBase> & constraint)
    {
        constraint = constraints_.get(constraintName);
        if (!constraint)
        {
            PRINT_ERROR("No constraint with this name exists.");
            return hresult_t::ERROR_BAD_INPUT;
    ...
    hresult_t Model::generateModelFlexible()
    {
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            for (const FlexibleJointData & flexibleJoint :
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            flexibleJointIndices_.clear();
            getJointIndices(pncModelFlexibleOrig_, flexibleJointNames_, flexibleJointIndices_);
    ...
    hresult_t Model::generateModelBiased(const uniform_random_bit_generator_ref<uint32_t> & g)
    {
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            for (const std::string & jointName : rigidJointNames_)
            {
                const pinocchio::JointIndex jointIndex = pinocchioModel_.getJointId(jointName);
            ...
            pinocchio::forwardKinematics(pinocchioModel_,
                                         pinocchioData_,
                                         qNeutral,
                                         Eigen::VectorXd::Zero(pinocchioModel_.nv));
    ...
    hresult_t Model::refreshProxies()
    {
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            getJointIndices(pinocchioModel_, rigidJointNames_, rigidJointIndices_);
            getJointsPositionIndices(
                pinocchioModel_, rigidJointNames_, rigidJointPositionIndices_, false);
            getJointsVelocityIndices(
                pinocchioModel_, rigidJointNames_, rigidJointVelocityIndices_, false);
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            if (modelOptions_->joints.enablePositionLimit)
            {
                ...
                else
                {
                    for (std::size_t i = 0; i < rigidJointPositionIndices_.size(); ++i)
                    {
                        ...
                        positionLimitMin_[positionIndex] =
                            modelOptions_->joints.positionLimitMin[i];
                        positionLimitMax_[positionIndex] =
                            modelOptions_->joints.positionLimitMax[i];
                    }
                }
            }
            ...
            if (modelOptions_->joints.enableVelocityLimit)
            {
                if (modelOptions_->joints.velocityLimitFromUrdf)
                {
                    ...
                    {
                        velocityLimit_[velocityIndex] =
                            pinocchioModel_.velocityLimit[velocityIndex];
                    }
                }
                else
                {
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            returnCode = refreshConstraintProxies();
        }
    ...
    hresult_t Model::refreshGeometryProxies()
    {
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            if (modelOptions_->dynamics.enableFlexibleModel)
            {
                for (auto && model : std::array{&collisionModel_, &visualModel_})
                {
                    for (pinocchio::GeometryObject & geom : model->geometryObjects)
                    {
                        ...
                        for (pinocchio::JointIndex i = newParentModelIndex;
                             i > oldParentModelIndex;
                             i = pinocchioModel_.parents[i])
                        {
                            geomPlacementRef =
                                pinocchioModel_.jointPlacements[i] * geomPlacementRef;
                        }
            ...
            for (hpp::fcl::CollisionRequest & collisionRequest : collisionData_.collisionRequests)
            {
                collisionRequest.num_max_contacts =
                    modelOptions_->collisions.contactPointsPerBodyMax;
            }
            ...
            getFrameIndices(pinocchioModel_, collisionBodyNames_, collisionBodyIndices_);
        }

        return returnCode;
    }

    hresult_t Model::refreshContactProxies()
    {
        hresult_t returnCode = hresult_t::SUCCESS;
        ...
        if (returnCode == hresult_t::SUCCESS)
        {
            ...
            // Extract the contact frames indices in the model
            getFrameIndices(pinocchioModel_, contactFrameNames_, contactFrameIndices_);
        }

        return returnCode;
    }

    hresult_t Model::refreshConstraintProxies()
    {
        hresult_t returnCode = hresult_t::SUCCESS;
        ...
        constraints_.foreach(
                ...
                ConstraintNodeType /* node */)
            {
                if (returnCode == hresult_t::SUCCESS)
                {
                    ...
                    returnCode = constraint->reset(pinocchio::neutral(pinocchioModel_),
    ...
    hresult_t Model::setOptions(GenericConfig modelOptions)
    {
        ...
        {
            PRINT_ERROR("The number of contact points by collision pair 'contactPointsPerBodyMax' "
                        "must be at least 1.");
            return hresult_t::ERROR_BAD_INPUT;
        }
        if (modelOptions_ &&
    ...
    hresult_t Model::getFlexiblePositionFromRigid(const Eigen::VectorXd & qRigid,
    ...
    hresult_t Model::getRigidPositionFromFlexible(const Eigen::VectorXd & qFlex,
